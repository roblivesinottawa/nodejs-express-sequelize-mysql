...
exports.findAll = (req, res) => {
	...
	.then(data => res.send(data))
	.catch(err => res.status(500).send({ message: err.message || "Some error occurred while trying to retrieve tutorials."}))
...
exports.findOne = (req, res) => {
	...
	.then(data => res.send(data))
	.catch(err => res.status(500).send({ message: err.message || `Some error occurred trying to retrive tutorial with id ${id}`}))
...
exports.delete = (req, res) => {
	...
	res.send({ message: `Cannot delete tutorial with the id=${id}. Maybe tutorial was not found.`})
}

})
	.catch(err => res.status(500).send({message: `Could nt delete tutorial with id=${id}`}))
};
